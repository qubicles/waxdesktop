...
const EXPLORERS = [
    {
        name:'TELOS Monitor',
        account:account => `http://testnet.telosfoundation.io/account/${account.name}`,
        transaction:id => `http://testnet.telosfoundation.io/transaction/${id}`,
        block:id => `http://testnet.telosfoundation.io/block/${id}`
    },
    {
        name:'LOCAL',
        account:account => `http://localhost:3039/account/${account.name}`,
        transaction:id => `http://localhost:3039/tx/${id}`,
        block:id => `http://localhost:3039/block/${id}`
    }
];
...
export default class EOS{
    ...
    //     const eosTokens = await fetch("https://raw.githubusercontent.com/qubicles/waxdesktop/master/resources/tokens.json").then(res => res.json()).catch(() => []);
    //     eosTokens.map(token => {
