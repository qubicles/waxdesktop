...
class ContractInterfaceFormAction extends Component<Props> {
  ...
  onSubmit = () => {
    const { actions, contract, contractAction, settings } = this.props;
  ...
  render() {
    ...
    const { currentArrayValues } = this.state;
    ...
    if (
      ...
      modal = (
        <GlobalTransactionModal
          ...
          content={<WalletModalContentBroadcast actions={actions} />}
    ...
    if (
      system.TRANSACTION_BROADCAST === "SUCCESS" &&
      transaction &&
      transaction.data &&
      transaction.data.transaction_id
    ) {
    ...
    if (
      system.TRANSACTION_SIGN === "FAILURE" &&
      transaction &&
      transaction.data &&
      transaction.data.transaction_id
    ) {
    ...
    return (
      <Form
        ...
        className="contract-actionParam-form"
      >
        <Header>
        ...
        <p>{t("interface_form_action_instructions")}</p>
        {/* <Divider /> */}
        {modal}
        {formFields}
        {errors}
