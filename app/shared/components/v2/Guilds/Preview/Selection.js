...
export default class ProducersVotingPreviewSelection extends Component<Props> {
  render() {
    ...
    const rows = chunk(
      times(selected.length, i => (
        <Table.Cell key={i}>{selected[i]}</Table.Cell>
      )),
      4
    );
    ...
    if (lastRow && lastRow.length < 4) {
      times(4 - lastRow.length, i => {
        lastRow.push(<Table.Cell width={4} key={`blank-${i}`} />);
      });
    }
    return (
      <I18n ns="producers">
        {t => (
          <Segment loading={submitting}>
            <Header
              icon="alarm"
              content={t("producer_voter_preview_confirm_changes_title")}
            />
            <Modal.Content>
              <h3>{t("producer_voter_preview_confirm_changes_message")}</h3>
                  ...
                  <Table.Body>
                    {selected.length > 0 ? (
                      ...
                      <Grid.Column textAlign="center" width={12}>
                        {t("producer_voter_preview_confirm_none")}
                      </Grid.Column>
                    )}
                  </Table.Body>
                </Table>
              ...
              />
              <Divider />
              <Button onClick={onClose} className="guild-cancel-btn">
