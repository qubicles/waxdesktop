...
export function undelegatebwParams(delegator, receiver, netAmount, cpuAmount, settings) {
  ...
  return {
    from: delegator,
    receiver,
    unstake_net_quantity: `${unstakeNetAmount.toFixed(8)} ` + settings.blockchain.tokenSymbol,
    unstake_cpu_quantity: `${unstakeCpuAmount.toFixed(8)} ` + settings.blockchain.tokenSymbol,
    transfer: 0
  };
}
