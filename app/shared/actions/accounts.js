...
export function claimUnstaked(owner) {
  return (dispatch: () => void, getState) => {
    ...
    return eos(connection, true, payforaction !== null)
      .transaction({
        actions: actions
      })
      .then(tx => {
        ...
        .then(results => {
          ...
          if (!modified.self_delegated_bandwidth) {
            modified.self_delegated_bandwidth = {
              cpu_weight:
                "0.".padEnd(settings.tokenPrecision + 2, "0") +
                " " +
...
export function getActions(account, start, offset) {
  return (dispatch: () => void, getState) => {
    const { connection, settings, actions } = getState();
    ...
    if (account && (settings.node || settings.node.length !== 0)) {
      eos(connection)
        .getActions(account, start, offset)
        .then(results => {
          const resultNewestAction =
            results.actions[results.actions.length - 1];
          const resultsNewestActionId =
            resultNewestAction && resultNewestAction.account_action_seq;

          const stateNewestAction = actionHistory.list[0];
          const stateNewestActionId =
            stateNewestAction && stateNewestAction.account_action_seq;
...
function uniqReqId(action, index, self) {
  ...
  return (
    self.map(actionItem => actionItem.account_action_seq).indexOf(actionId) ===
    index
  );
}
